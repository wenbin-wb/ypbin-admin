...
#   [3/7] 构建 starter —— mvn install（微服务依赖 starter 2.2.0 及新能力）
...
if [ "${SKIP_STARTER_BUILD:-0}" != "1" ]; then
  ...
  # 确认本地仓库有 2.2.0（没有则强制构建）
  if [ ! -d "$HOME/.m2/repository/cn/ypbin/ypbin-starter-core/2.2.0" ]; then
    warn "本地 Maven 仓库无 starter 2.2.0，强制构建"
    cd "$ROOT/ypbin-starter"
